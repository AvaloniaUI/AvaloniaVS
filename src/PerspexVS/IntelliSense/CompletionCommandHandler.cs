...
namespace PerspexVS.IntelliSense
{
    ...
    internal class CompletionHandlerProvider : IVsTextViewCreationListener
    {
        ...
        private class TestCompletionCommandHandler : IOleCommandTarget
        {
            ...
            internal TestCompletionCommandHandler(IVsTextView textViewAdapter, ITextView textView,
            ...
            public int Exec(ref Guid pguidCmdGroup, uint nCmdID, uint nCmdexecopt, IntPtr pvaIn, IntPtr pvaOut)
            {
                ...
                bool handled = false;
                if (nCmdID == (uint)VSConstants.VSStd2KCmdID.COMPLETEWORD || (!typedChar.Equals(char.MinValue) && (char.IsLetterOrDigit(typedChar) || typedChar == '<' || typedChar == ' ')))
                {
                    if (typedChar != '\0')
